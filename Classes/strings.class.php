...
class Strings {
     ...
     */
    private static function montaStringCamelCase ($strOrigem,
                                                  $comecaComMaiusculo = false) {
    ...
    public static function trocaNomeColunaParaSetAtributo ($nomeDaColuna) {
        return "set" . self::montaStringCamelCase ($nomeDaColuna,
                                                   $comecaComMaiusculo = true);
    }
    ...
    public static function trocaNomeColunaParaGetAtributo ($nomeDaColuna) {
        return "get" . self::montaStringCamelCase ($nomeDaColuna,
                                                   $comecaComMaiusculo = true);
    }
...
//        return implode('', $nomeDoAtributo); // Transforma o array com o nome do atributo em uma string única.
    ...
    /**
     * Troca caracteres informados da string desejada.
     * 
     * @param array $caracteresASeremSubstituidos Array com os caracteres a 
     * serem retirados da string.
     * @param array $caracteresSubstitutos Array com os caracteres a serem 
     * substituídos. Os caracteres precisam estar na mesma ordem dos caracteres 
     * a serem substituídos.
     * @param type $stringASerAlterada String a ser realizada a troca de caracteres.
     * @return type String com os caracteres trocados.
     */
    public static function trocaCaracteresDaString (Array $caracteresASeremSubstituidos,
                                                    Array $caracteresSubstitutos,
                                                    $stringASerAlterada) {
        if (is_null ($stringASerAlterada)) {
            return null;
        }

        $arrayString = str_split ($stringASerAlterada); // Fatia a string e devolve um array
        $novaString  = Array();
        foreach ($arrayString as $letra) {
            if (in_array ($letra, $caracteresASeremSubstituidos)) {
                // Se o caractere da string é um caractere que se deseja substituir, faz-se a troca:
                $i = array_search ($letra, $caracteresASeremSubstituidos);
                if ($caracteresASeremSubstituidos [$i] == chr (13) && $caracteresASeremSubstituidos[$i - 1] == chr (10)) {
                    //no caso de enter tem-se dois caracteres: 13 e 10 nessa 
                    //ordem, daí só precisa inserir um <br> e pode-se pular este.
                } else {
                    $novaString [] = $caracteresSubstitutos [$i];
                }
            } else {
                // Se for um caractere desejado mantém ele
                $novaString [] = $letra;
